...
public class LevelModel implements MessageListener
{
    ...
    Timer boidSpawnTimer = new Timer();
    ...
    private void buildLevel(FileHandle levelFile)
    {
        ...
        //set the time for time attack mode
    ...
    public void update(float delta)
    {
        ...
        else if(levelType == LEVEL_TYPE.ENDLESS && boidSpawnTimer.isEmpty())
        {
            
            boidSpawnTimer.scheduleTask(new Timer.Task()
            {
                @Override
                public void run()
                {
                    for (int i = 0; i < 15; i++)
                    {
                        Boid b = new Boid(
                                    new Vector2(
                                            MathUtils.random(worldModel.WORLD_WIDTH), 
                                            MathUtils.random(worldModel.WORLD_HEIGHT)));
                        
                        worldModel.addEntity(b);
